...
def ticcmd(self, *args, debug=False):
    logger.debug("ticcmd -d " + self.serialnumber + ' ' + (" ".join(list(args))))
    return str(['ticcmd'] + list(args) + ['-d'] + [self.serialnumber])
...
class _TicController:
    ...
    def energize(self):
        ...
        """
        return ticcmd(self,"--energize", '--resume', debug=self.debug)
    ...
    def deenergize(self):
        ...
        """
        return ticcmd(self,"--deenergize", debug=self.debug)
    ...
    def exit_safe_start(self):
        ...
        """
        return ticcmd(self,"--exit-safe-start", debug=self.debug)
    ...
    def enter_safe_start(self):
        ...
        """
        return ticcmd(self,"--enter-safe-start", debug=self.debug)
    ...
    def _set_target_position(self, position):
        ...
        """
        #return ticcmd(self,"--position", str(position), debug=self.debug)
        return ticcmd(self,"--resume","--position", str(position), debug=self.debug)
    ...
    def _set_target_velocity(self, velocity):
        ...
        These controls are on the “Status” tab.
  """
        return ticcmd(self,"--velocity", str(velocity), debug=self.debug)
    ...
    def _halt_and_set_position(self, position):
        ...
        """
        return ticcmd(self,"--halt-and-set-position", str(position), debug=self.debug)
    ...
    def _halt_and_hold(self):
        ...
        """
        return ticcmd(self,"--halt-and-hold", debug=self.debug)
    
    def home(self, direction=1):
        home_dir = "fwd" if direction == 1 else "rev"
        return ticcmd(self,"--home", home_dir, debug=self.debug)
    ...
    def reset_command_timeout(self):
        ...
        """
        return ticcmd(self,"--reset-command-timeout", debug=self.debug)  
    ...
    def _reset(self):
        ...
        """
        return ticcmd(self,"--reset", debug=self.debug)
    ...
    def _set_max_velocity(self, velocity):
        ...
        """
        return ticcmd(self,"--max-speed", str(velocity), debug=self.debug)
    ...
    def _set_starting_velocity(self, velocity):
        ...
        (or Reinitialize) command or full microcontroller reset.        
        """
        return ticcmd(self,"--starting-speed", str(velocity), debug=self.debug)
    ...
    def _set_max_acceleration(self, accel):
        ...
        """
        return ticcmd(self,"--max-accel", str(accel), debug=self.debug)
    ...
    def _set_max_deceleration(self, accel):
        ...
        """
        return ticcmd(self,"--max-decel", str(accel), debug=self.debug)
    ...
    def _set_step_mode(self, step_size):
        ...
        (or Reinitialize) command or full microcontroller reset.
        """
        return ticcmd(self,"--step-mode", str(step_size), debug=self.debug)

    def _set_current_limit(self, current):
        """
        This command temporarily sets the current limit in mA.
        The provided value will override the corresponding setting from the Tic’s non-volatile memory until the next Reset 
        (or Reinitialize) command or full microcontroller reset.
        """
        return ticcmd(self,"--current", str(current), debug=self.debug)
    ...
    def _get_data(self, args):
        ...
        """
        if args == '':
            return ticcmd(self, "--status", debug=self.debug)
        else:
            return ticcmd(self, "--status", args, debug=self.debug)
...
class TicController(_TicController):
    ...
    def __init__(self, serialnumber='', current_limit=100, max_accel=1000000,max_deccel=0,max_speed=100000000,start_speed=0,
                min_position=-200000, max_position=200000, 
                velocity=1000000, move_size=200, step_size=StepSizes.FULL, 
                power_up_down=True, safe_start=True, debug=True):
        ...
        self.safe_start = safe_start
        #set velocity, current and other params
        self._set_current_limit(current_limit)
        self._set_max_acceleration(max_accel)
        self._set_max_deceleration(max_deccel)
        self._set_max_velocity(max_speed)
        self._set_starting_velocity(start_speed)
    ...
    def move(self, new_position):
        if self.min_position < new_position < self.max_position:
            ...
            #this doesn't work, the movement is stopped (power down command) before the stepper reaches its destination.
    ...
    def get_status_data(self):
        return self._get_data('--full')

    def get_pos(self):
        return self._get_data('')

    def perform_homing(self,dir=1):
